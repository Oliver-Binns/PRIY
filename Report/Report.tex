...
Attempting to do so with current technology would also be completely intractable.
...
The platform model refines the domain model and states how non-deterministic biological behaviour can be modelled \gls{in-silico}.
...
The original \gls{platform} will be further refined in our \gls{FLAME GPU} implementation.
...
X-machines are structurally identical to finite state machines, other than that they have memory, which means it was possible to directly implement most of the platform model state diagrams directly into the \gls{FLAME GPU} model (Fig. \ref{fig:gmf_flame_generation}).
...
A number of other differences were made to the platform model, mainly because of the time limitations of the project.
...
The platform model diagram for \gls{LTi} cells has a missing transition.
\gls{LTi} cells may collide with \gls{LTo} cells, without reacting to chemokine emission.
In this case the \gls{LTi} cell would transition directly from its random movement state, to the contact with \gls{LTo} state.
...
The migration of \gls{LTi}(n) cells is managed by a host function.
The ability to create agents from a host function is currently only available in a pre-release version of \gls{FLAME GPU}, which is why this version was selected.
...
%INPUT FILE: how is the set up?
In this platform model there is a single \textit{active} \gls{LTo} cell in the centre of the environment.
%additional LTi cell to set environment bounds
%likely to leave the environment, due to random movement.
%will not have a significant impact on the simulation behaviour.
...
It has not been verified by a domain expert.
\begin{figure}[htp]
\begin{subfigure}{\textwidth}
\centering
\includegraphics[width=\textwidth]{Appendix/Sim/Start}
\caption{Beginning of simulation, one central \gls{LTo} cell is present.}
\label{fig:sim_start}
\end{subfigure}
