...
In the tool \gls{FLAME GPU} models are designed using a graphical interface (Figs. \ref{fig:ltin_gmf}, \ref{fig:ppsim_gmf}) and then exported to the required XML file format.
...
As \gls{LTo} cells do not constantly release chemokines, it is possible for the cell to make contact with an \gls{LTo} cell without first transitioning through the response to chemokine state.
...
\centering
\includegraphics[width=\textwidth]{Appendix/LTo_Trace}
...
\caption{Traceability from \gls{domain} to \gls{FLAME GPU} Implementation}
