...
%ABM...
In particular this project will focus on simulations created using Agent-Based Modelling (ABM).
...
GPU normally only found in gaming PCs and consoles
...
\caption{FLAME Development Process (Taken from \cite{flame_simulation})}
\label{fig:flame_dev}
\end{wrapfigure}

Building FLAME simulations requires the creation of two artifacts (Fig. \ref{fig:flame_dev}), a structural `XML Model File' which defines the agents in the system and their allowed channels for communication and a C source code file of `Scripted Behaviour' which defines the actual behaviour of agents.
\\
...
As FLAME GPU is developed openly, this version is freely available online\cite{flame_github}. FLAME has been added to the project repository as a submodule, to ensure version compatibility between FLAME and PPSim v2.
