...
In this project 
the \acrshort{flamegpu} framework is used to help mitigate some of the difficulties with implementing parallel programs, in order to build on an existing sequential \acrshort{mason} simulation.
I also propose \acrfull{mde} as a potential short-term solution to the skills shortage, exploring some of the current \acrshort{mde} technologies for creating simulations and proposing further advancements to these tools for the future.
}

\acknowledgements{I would like to my supervisors, Fiona Polack and Kieran Alden, for their support and guidance throughout this project. I would also like to thank the \acrshort{flamegpu} development team at the University of Sheffield for their help in utilising the latest and pre-released features of this framework.}
...
\newacronym{flamegpu}{FLAME GPU}{An extension of the \acrshort{flame} framework which uses \acrshort{gpu}s for parallelism}
\newacronym{gpu}{GPU}{Graphics Processing Unit}
\newacronym{gpgpu}{GPGPU}{General Purpose \acrshort{gpu}}
\newacronym{mason}{MASON}{A Java framework for producting simulations using \acrshort{abm}}
...
\acrshort{flamegpu} takes input in the form of an XMML model which defines the types of agents that feature in the simulation and their interactions%read XMML paper!
%x-machine, etc.
